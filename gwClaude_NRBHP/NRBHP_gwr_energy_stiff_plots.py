"""
Plotting script for the gwr_energy_stiff model (9 coefficients, globally jointly fitted).

Run fit_scaling_gwr_energy_stiff.py --global --maxiter 80 first.
Plots written to Agentic_plots/gwr_energy_stiff/ (always saved).
...
differs: the four per-q parameters come from stiff.params_at (alpha_PP deg-3 in nu,
alpha_E = A(nu)/nu, beta_PP deg-2 in X2 = 1/(1+q), beta_E = P(nu)/beta_PP) instead of
four free cubics in nu.

Model waveforms come from fit_scaling_gw_remnant_energy.evaluate_model, i.e. the same
evaluator that produced the mismatches in scaling_gwr_energy_stiff.md, so the figures
...
PLOT_DIR = ROOT / "Agentic_plots" / "gwr_energy_stiff"
PLOT_DIR.mkdir(parents=True, exist_ok=True)
COEFF_JSON = stiff.COEFFS


def read_coefficients(path: Path = COEFF_JSON):
    d = json.loads(path.read_text())
    coeffs = {k: np.asarray(v, float) for k, v in d.items() if not k.startswith("_")}
    ncoef = int(d.get("_meta", {}).get("n_coefficients", sum(
        stiff.SPEC[n][1] + (0 if stiff.SPEC[n][2] is not None else 1) for n in stiff.NAMES)))
    return coeffs, ncoef
...
def build_model(q, coeffs):
    ...
    params = stiff.params_at(q, coeffs)
...
def _title(q, error=float("nan")):
    ...
    return f"gwr-energy-stiff  q={q}{ex};  nu={gwre.get_nu(q):.4f}{es}"
...
def save_waveform_plot(q, common_t, h_nr, h_model, error):
    ...
    for ax, xlim in [(ax1, (-1000, split)), (ax2, (split, 100))]:
        ...
        ax.plot(common_t, np.real(h_model), label="BHPT gwr-energy-stiff", alpha=0.8)
        ax.set_xlim(*xlim); ax.grid(True); ax.set_xlabel("t/M")
    ax1.legend(fontsize=8)
    fig.suptitle(_title(q, error)); fig.tight_layout()
    path = PLOT_DIR / f"gwr_stiff_{_tag(q)}_waveform.pdf"
...
def save_zoomed_plot(q, common_t, h_nr, h_model):
    ...
    path = PLOT_DIR / f"gwr_stiff_{_tag(q)}_zoomed.pdf"
    fig.savefig(path); plt.close(fig); return path
...
def save_parameter_plot(q, tau, alpha, beta, e_oft, error, t_lo, t_hi):
    ...
    with plt.rc_context(_PARAM_RC):
        ...
        fig.tight_layout()
        path = PLOT_DIR / f"gwr_stiff_{_tag(q)}_params.pdf"
        fig.savefig(path)
        plt.close(fig)
    return path
...
def save_coefficients_vs_q(coeffs, ncoef):
    ...
    master = np.array([stiff.params_at(float(x), coeffs)[:4] for x in q_fine])
    ...
    fig.suptitle("gwr-energy-stiff: master coefficients vs q")
    fig.tight_layout()
    path = PLOT_DIR / "gwr_stiff_coefficients.pdf"
    fig.savefig(path); plt.close(fig); return path
...
def save_alpha_beta_overlay(results):
    ...
    fig.suptitle("gwr-energy-stiff: master alpha/beta across q"); fig.tight_layout()
    path = PLOT_DIR / "gwr_stiff_alpha_beta_overlay.pdf"
    fig.savefig(path); plt.close(fig); return path
...
def save_energy_vs_q(results):
    ...
    for ax, yl, tl in [(ax1, "E(t) [M]", "gw_remnant radiated energy"),
        ...
        ax.set_xlabel("t_NR / M"); ax.set_ylabel(yl); ax.set_title(tl); ax.legend(fontsize=8)
    fig.suptitle("gwr-energy-stiff: the E(t) coordinate across q")
    fig.tight_layout()
    path = PLOT_DIR / "gwr_stiff_Eoft_vs_q.pdf"
    fig.savefig(path); plt.close(fig); return path
