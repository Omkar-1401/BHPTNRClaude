"""
Plotting script for the wf_nu_hybrid_q_dep model.

Run fit_scaling_wf_nu_hybrid_q_dep.py first.

Plots are written to Agentic_plots/wf_nu_hybrid/ ONLY IF the q=2
extrapolation error is below 1e-2.  If the threshold is not met,
the summary is still printed but no files are created.
...
PLOT_DIR  = ROOT / "Agentic_plots" / "wf_nu_hybrid"
...
def read_markdown_coefficients(md_path: Path) -> dict[str, np.ndarray]:
    ...
    missing = [n for n in wfhyb.PARAM_NAMES if n not in coeffs]
    if missing:
        raise ValueError(f"Missing coefficient rows: {missing}")
    return {n: coeffs[n] for n in wfhyb.PARAM_NAMES}
...
def build_model(q: float, coeffs: dict[str, np.ndarray]) -> dict:
    ...
    params = wfhyb.constrained_master_params(q, coeffs)
    params, _ = wfhyb.polish_nuisance(params, case)
    ev = wfhyb.evaluate_model_hybrid(
        params,
...
def save_waveform_plot(q, common_t, h_nr, h_model, error) -> Path:
    split = -200.0
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 4),
                                    gridspec_kw={"width_ratios": [3, 2]})
    for ax, xlim in [(ax1, (-1000, split)), (ax2, (split, 100))]:
        ax.plot(common_t, np.real(h_nr),    label="NR 22 mode")
        ax.plot(common_t, np.real(h_model), label="BHPT hybrid", alpha=0.8)
        ax.set_xlim(*xlim); ax.grid(True); ax.set_xlabel("t/M")
    ax1.legend(fontsize=8)
    fig.suptitle(_title(q, error))
    fig.tight_layout()
    path = PLOT_DIR / f"hybrid_{_tag(q)}_waveform.pdf"
    fig.savefig(path); plt.close(fig)
    return path


def save_parameter_plot(q, tau_use, alpha_use, beta_use, e_hat_use, j_hat_use) -> Path:
    ab    = bhpt_utils.all_alpha_beta_1D_scaling_params(q, [creative.MODE])
    nu_val = wfhyb.get_nu(q)
    label  = f"wf-nu-hybrid  nu={nu_val:.4f}"
    ...
    axes[0].set_xlabel("t_NR / M"); axes[0].set_ylabel("alpha(t)")
    ...
    axes[1].set_xlabel("t_NR / M"); axes[1].set_ylabel("beta(t)")
    ...
    axes[2].set_title("alpha vs Ehat (waveform flux)")
    axes[2].grid(True)

    fig.suptitle(_title(q))
    fig.tight_layout()
    path = PLOT_DIR / f"hybrid_{_tag(q)}_params.pdf"
    fig.savefig(path); plt.close(fig)
    return path


def save_loss_coord_plot(q, t_bhpt, losses) -> Path:
    fig, axes = plt.subplots(1, 3, figsize=(14, 4))
    mask = (t_bhpt >= -1500) & (t_bhpt <= 150)
    for key, ax, label in [
        ("e_hat",  axes[0], "Ehat"),
        ("j_hat",  axes[1], "Jhat"),
        ("p_loss", axes[2], "p_loss"),
    ]:
        ax.plot(t_bhpt[mask], losses[key][mask], lw=1)
        ...
        ax.grid(True)
    fig.suptitle(f"wf-nu-hybrid loss coordinates  q={q}  nu={wfhyb.get_nu(q):.4f}")
    fig.tight_layout()
    path = PLOT_DIR / f"hybrid_{_tag(q)}_loss_coords.pdf"
    fig.savefig(path); plt.close(fig)
    return path
...
def save_zoomed_plot(q, common_t, h_nr, h_model) -> Path:
    ...
    fig.tight_layout()
    path = PLOT_DIR / f"hybrid_{_tag(q)}_zoomed.pdf"
    fig.savefig(path); plt.close(fig)
    return path


def save_coefficient_table(coeffs) -> Path:
    degree = len(next(iter(coeffs.values()))) - 1
    columns    = ["parameter", "anchor", *[f"c{i}" for i in range(degree + 1)], "value q=5"]
    table_rows = []
    for name in wfhyb.PARAM_NAMES:
        anc     = wfhyb.PP_ANCHORS[name]
        anc_str = f"{anc:.4g}" if anc is not None else "free"
        val     = wfhyb.eval_poly(5.0, coeffs[name])
    ...
    ax.set_title("wf-nu-hybrid: polynomial coefficients (nu basis, PP-anchored)")
    fig.tight_layout()
    path = PLOT_DIR / "hybrid_coefficients.pdf"
    fig.savefig(path); plt.close(fig)
    return path


def save_coupling_vs_q(coeffs) -> Path:
    q_fine = np.linspace(1.5, 9.0, 250)
    names  = ["alpha_L", "alpha_E", "alpha_J", "beta_r", "beta_L"]

    fig, axes = plt.subplots(1, len(names), figsize=(18, 4))
    for ax, name in zip(axes, names):
        vals = [wfhyb.eval_poly(float(q), coeffs[name]) for q in q_fine]
    ...
    fig.suptitle("wf-nu-hybrid coupling parameters vs q  (gray dashed = training boundary)")
    fig.tight_layout()
    path = PLOT_DIR / "hybrid_couplings_q.pdf"
    fig.savefig(path); plt.close(fig)
    return path


def save_alpha_beta_vs_q(coeffs) -> Path:
    q_fine = np.linspace(1.5, 9.0, 250)
    fig, axes = plt.subplots(1, 3, figsize=(14, 4))
    for ax, name, ylabel in [
        (axes[0], "alpha_i", "alpha_i (amplitude factor)"),
        (axes[1], "beta_i",  "beta_i (inspiral time-stretch)"),
        (axes[2], "beta_r",  "beta_r (ringdown time-stretch)"),
    ]:
        vals = [wfhyb.eval_poly(float(q), coeffs[name]) for q in q_fine]
        ...
        ax.set_title(f"{name}(q)")
        ax.grid(True); ax.legend(fontsize=7)
    fig.suptitle("wf-nu-hybrid: amplitude/time-stretch parameters vs q")
    fig.tight_layout()
    path = PLOT_DIR / "hybrid_alpha_beta_q.pdf"
    fig.savefig(path); plt.close(fig)
    return path


def save_alpha_L_vs_q(coeffs) -> Path:
    q_fine = np.linspace(1.5, 9.0, 250)
    vals   = [wfhyb.eval_poly(float(q), coeffs["alpha_L"]) for q in q_fine]

    fig, ax = plt.subplots(figsize=(7, 4))
    ax.plot(q_fine, vals, lw=1.5, label="alpha_L polynomial")
    ...
    ax.set_xlabel("q"); ax.set_ylabel("alpha_L")
    ax.set_title("Inspiral alpha drift coefficient alpha_L(q)")
    ax.legend(fontsize=8); ax.grid(True)
    fig.tight_layout()
    path = PLOT_DIR / "hybrid_alpha_L_q.pdf"
    fig.savefig(path); plt.close(fig)
    return path
...
def main() -> None:
    ...
    for q in Q_INPUTS:
        ...
        for i, name in enumerate(wfhyb.PARAM_NAMES):
            print(f"  {name:<10} = {params[i]:.6g}")
    ...
    q2_err = errors.get(2.0, float("inf"))
    if q2_err >= Q2_THRESH:
        print(f"\nq=2 mathcalE = {q2_err:.4e} >= {Q2_THRESH:.0e}: plots NOT saved.")
        return

    print(f"\nq=2 mathcalE = {q2_err:.4e} < {Q2_THRESH:.0e}: saving plots to {PLOT_DIR} ...")
