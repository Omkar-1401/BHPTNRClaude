...
PLOT_DIR = ROOT / "Notebook_plots" / "Two_constant_q_dep"
...
import fit_scaling_two_constant_qdep as twoq  # noqa: E402
...
Q_INPUT = 6.0
MODEL_TAG = "two_constant_qdep_q6"
TITLE = "Two-constant q-dependent Agentic fit (q = 6)"
...
def build_model() -> tuple[dict[str, np.ndarray], dict, np.ndarray, dict, tuple]:
    coeffs = read_markdown_coefficients(
        MD_PATH, "## Selected Master Coefficients", twoq.PARAM_NAMES
    )
    nrsur = gwsurrogate.LoadSurrogate("NRHybSur3dq8")
    case = twoq.baseq.load_case(nrsur, Q_INPUT, source_stride=3, nr_stride=8)
    params = twoq.master_params(Q_INPUT, coeffs)
    params, _fit_error = twoq.polish_time_phase(params, case)
    eval_data = twoq.evaluate_case(params, case, subset="full", min_coverage=0.84)
    arrays = twoq.model_arrays(params, case["t_bhpt"], case["h_bhpt"], case["meta"])
    return coeffs, case, params, eval_data, arrays


def t_cut_nr(params: np.ndarray, meta: dict) -> float:
    p = twoq.unpack(params)
...
def save_waveform_plot(common_t: np.ndarray, h_nr: np.ndarray, h_bhpt: np.ndarray, split_t: float, error: float) -> Path:
    split = float(np.clip(split_t, -900.0, -50.0))
...
def save_parameter_plot(tau: np.ndarray, alpha: np.ndarray, beta: np.ndarray) -> Path:
    ab = bhpt_utils.all_alpha_beta_1D_scaling_params(Q_INPUT, [twoq.MODE])
    fig, (ax1, ax2) = plt.subplots(
        nrows=1,
        ncols=2,
        figsize=(10, 4),
    ...
    ax1.plot(tau, alpha, label="Two-constant q-dependent")
    ax1.plot(
        tau,
    ...
    ax2.plot(tau, beta, label="Two-constant q-dependent")
    ax2.plot(
        tau,
...
def save_coefficient_table(coeffs: dict[str, np.ndarray]) -> Path:
    ...
    columns = ["coefficient", *[f"c{i}" for i in range(degree + 1)], "value q=6"]
    rows = []
    for name in twoq.PARAM_NAMES:
        values = coeffs[name]
        rows.append(
